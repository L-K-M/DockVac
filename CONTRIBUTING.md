...
To run the app without Docker, replay the fixtures with `scripts/fake-docker.py --socket /tmp/dockvac-fake.sock` and launch the binary with `DOCKER_HOST=unix:///tmp/dockvac-fake.sock`. Setting `DOCKVAC_SMOKE_SCRIPT=focus-images,select-first,add-safe,review` walks through the screens after the first scan; the UI Smoke workflow uses this to capture screenshots. Nothing is ever confirmed or removed by the script.
